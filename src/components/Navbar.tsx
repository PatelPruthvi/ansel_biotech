...
const productHrefs = new Set([
  "/products",
  ...productCategories.map((p) => p.href),
]);
...
function isProductPath(path: string) {
  if (isIndustryPath(path)) return false;
  return productHrefs.has(path);
}

const navLinks = [
  { name: "Home", href: "/" },
  { name: "About Us", href: "/about" },
  { name: "Products", href: "/products", activeWhen: "products" as const },
  { name: "Industries", href: "/industries", activeWhen: "industries" as const },
  { name: "Contact", href: "/contact" },
];

function isActiveHref(location: string, link: (typeof navLinks)[number]) {
  if (link.activeWhen === "products") return isProductPath(location);
  if (link.activeWhen === "industries") return isIndustryPath(location);
...
export function Navbar() {
  ...
  const measurePill = () => {
    ...
    if (!wrap) return;
    const activeIdx = navLinks.findIndex((l) => isActiveHref(location, l));
    ...
    >
      <div className="max-w-[1160px] mx-auto px-3 sm:px-4 lg:px-5 flex items-center justify-between gap-4">
        ...
        >
          ...
          {navLinks.map((link, i) => {
            const isActive = isActiveHref(location, link);
            return (
              <Link
                key={link.href}
                href={link.href}
                ...
                className={`relative z-10 px-4 py-[7px] rounded-full font-sans text-[0.66rem] tracking-[0.1em] uppercase transition-colors whitespace-nowrap ${
                  isActive ? "text-white" : "text-fg-m hover:text-foreground"
                }`}
              >
                {link.name}
              </Link>
          ...
          <div className="w-[1px] h-[18px] bg-border-m mx-[3px]" />
          <button
            onClick={toggleTheme}
      ...
      {mobileMenuOpen && (
        ...
        >
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                className={`px-3.5 py-2.5 rounded-lg font-sans text-[0.68rem] tracking-[0.1em] uppercase transition-colors ${
                  isActive
          ...
          >
            <span className="font-sans text-[0.68rem] tracking-[0.12em] uppercase text-fg-m">
              Theme
            </span>
            <div
              ...
              style={{
                background:
                  theme === "dark" ? "rgba(106,178,32,0.35)" : "rgba(0,0,0,0.15)",
              }}
            >
              <div
                ...
                style={{ left: theme === "dark" ? "24px" : "2px" }}
              >
                {theme === "dark" ? "🌙" : "☀️"}
              </div>
            </div>
